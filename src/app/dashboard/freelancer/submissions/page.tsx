...
> = {
  ...
  under_review: { icon: Eye, color: "text-[#ef233c]", bg: "bg-[#ef233c]/10", label: "Under Review" },
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold font-[var(--font-heading)] flex items-center gap-2">
              <FileCheck className="w-6 h-6 text-emerald-400" /> My Submissions
            </h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-vault-card border border-white/10">
            ...
            {(["pending", "under_review", "approved", "rejected"] as const).map((status) => {
              ...
              return (
                <button
                  ...
                  className={`border border-white/10 bg-zinc-900/50 backdrop-blur-sm rounded-xl p-4 text-center transition-all duration-200 hover:scale-[1.02] ${
                    filter === status ? "ring-2 ring-vault-purple/50 border-[#ef233c]/30" : ""
                  }`}
                >
                  <StatusIcon className={`w-5 h-5 mx-auto mb-2 ${cfg.color}`} />
                  <p className={`text-2xl font-[var(--font-heading)] font-bold ${cfg.color}`}>
                    {counts[status]}
                  </p>
      ...
      {filtered.length === 0 ? (
        <ScrollReveal delay={0.15}>
          <div className="border border-white/10 bg-zinc-900/50 backdrop-blur-sm p-10 text-center rounded-2xl">
            <FileCheck className="w-12 h-12 text-zinc-500 mx-auto mb-4" />
            ...
            {filtered.map((sub, i) => {
              ...
              return (
                <Link key={sub.id} href={`/bounties/${sub.bounty_id}`}>
                  <div
                    className="border border-white/10 bg-zinc-900/50 backdrop-blur-sm p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center gap-4 group cursor-pointer hover:border-white/10-hover transition-all duration-300"
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <p className="text-xs text-zinc-500 mt-0.5 flex items-center gap-1 justify-end group-hover:text-[#ef233c] transition-colors">
                        <ExternalLink className="w-3 h-3" /> View Bounty
                      </p>
                      {sub.status === "rejected" && (
                        <p className="text-[10px] text-amber-400 flex items-center gap-1 justify-end mt-1">
