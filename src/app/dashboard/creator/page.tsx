  ...
  const statCards = [
    {
      ...
      color: "text-vault-purple-light",
      bg: "bg-vault-purple/10",
    },
    {
      icon: Zap,
      label: "Active Bounties",
      value: stats.active_bounties,
    ...
    },
    {
      icon: DollarSign,
      label: "Total Spent",
      value: `${stats.total_spent_algo.toFixed(1)}`,
      suffix: "ALGO",
    ...
    },
    {
      ...
      value: stats.pending_acceptances,
    ...
    <div className="space-y-6 py-4">
      ...
      <ScrollReveal>
        <div className="relative glass-card rounded-3xl p-6 sm:p-8 overflow-hidden">
          <div className="absolute -top-20 -right-20 w-64 h-64 rounded-full bg-vault-purple/10 blur-[80px] pointer-events-none" />
          <div className="absolute -bottom-10 -left-10 w-48 h-48 rounded-full bg-vault-cyan/8 blur-[60px] pointer-events-none" />
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-5">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-vault-purple to-vault-magenta flex items-center justify-center text-white text-xl font-bold shadow-lg shadow-vault-purple/20">
              {user?.avatar_url ? (
            ...
            <div className="flex flex-wrap gap-2">
              {user?.wallet_address ? (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full glass text-xs text-vault-text-secondary">
                  <Wallet className="w-3 h-3 text-vault-cyan" />
                  Connected
      ...
      <ScrollReveal delay={0.1}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {statCards.map((stat, i) => (
            <div
              key={stat.label}
              className="glass-card p-4 rounded-2xl text-center group"
          ...
          <Link href="/dashboard/creator/acceptances">
            <Button variant="secondary" size="sm">
              ...
              {stats.pending_acceptances > 0 && (
                <span className="ml-1 px-1.5 py-0.5 rounded-full bg-vault-red/20 text-vault-red text-[10px] font-bold">
      ...
      <ScrollReveal delay={0.2}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold font-[var(--font-heading)]">Recent Bounties</h2>
          <Link href="/dashboard/creator/bounties" className="text-sm text-vault-purple-light hover:underline flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {bounties.slice(0, 6).map((bounty, i) => (
              <Link key={bounty.id} href={`/bounties/${bounty.id}`}>
                <div
                  className="glass-card p-5 rounded-2xl flex flex-col group cursor-pointer h-full"
                ...
                >
                  ...
                  <p className="text-xs text-vault-text-secondary mb-3 line-clamp-2 flex-1">
                    {bounty.description}
                  </p>
        ...
        <ScrollReveal delay={0.25}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold font-[var(--font-heading)] flex items-center gap-2">
              <Bell className="w-5 h-5 text-vault-amber" />
              Notifications
              {stats.unread_notifications > 0 && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-vault-red/20 text-vault-red font-bold">
          ...
          <div className="space-y-2">
            {notifications.slice(0, 5).map((notif, i) => (
              <div
                key={notif.id}
                className={`glass-card p-4 rounded-xl flex items-start gap-3 transition-all ${
                  !notif.is_read ? "border-vault-purple/30 bg-vault-purple/5" : ""
                }`}
                style={{ animationDelay: `${i * 40}ms` }}
              >
                <div className="mt-1.5 flex-shrink-0">
                  {!notif.is_read ? (
                    <span className="block w-2 h-2 rounded-full bg-vault-purple animate-pulse" />
                  ) : (
                    <span className="block w-2 h-2 rounded-full bg-vault-text-muted/20" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                ...
                {notif.bounty_id && (
                  <Link href={`/bounties/${notif.bounty_id}`} className="flex-shrink-0 text-vault-purple-light hover:text-vault-purple">
