...
export function DashboardSidebar({ isMobileOpen, onClose }: DashboardSidebarProps) {
  ...
  // Combined sign out: disconnect wallet + logout
  ...
  const sidebarContent = (isMobile: boolean) => (
    <div className="flex-1 border border-white/10 bg-black rounded-xl p-3 flex flex-col overflow-hidden">
      ...
      {isMobile && (
        <button
          onClick={onClose}
          className="flex items-center justify-end p-2 mb-1 text-zinc-400 hover:text-white"
        >
          <X className="w-5 h-5" />
        </button>
      )}
      ...
      {/* User info */}
      <div className={`flex items-center gap-3 p-3 mb-2 rounded-xl bg-white/5 ${!isMobile && collapsed ? "justify-center" : ""}`}>
      ...
      {/* Wallet status */}
      <div className={`mt-2 px-3 py-2 rounded-xl bg-white/5 ${!isMobile && collapsed ? "text-center" : ""}`}>
      ...
      {/* Sign out — now disconnects wallet too */}
      <button
        onClick={handleSignOut}
        className={`mt-2 flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-zinc-500 hover:text-red-400 hover:bg-red-900/10 transition-all ${!isMobile && collapsed ? "justify-center" : ""}`}
      >
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside
        className={`hidden lg:flex sticky top-20 h-[calc(100vh-5rem)] flex-col transition-all duration-300 ${
          collapsed ? "w-16" : "w-64"
        }`}
      >
      ...
      {/* Mobile drawer */}
      <aside
        className={`lg:hidden fixed top-0 left-0 bottom-0 w-[280px] z-50 flex flex-col p-3 bg-black transition-transform duration-300 ease-in-out ${
